...
def validate_caption(caption: str, trigger_word: str) -> dict:
    """
    Validate a caption for correctness.
    ...
    # Check if caption starts with trigger word
    if not caption.startswith(trigger_word):
        errors.append(f"Caption must start with trigger word '{trigger_word}'")
    
    # Check caption length (should be 10-30 words)
    word_count = len(caption.split())
    if word_count < 5:
        warnings.append(f"Caption is too short ({word_count} words, recommended 10-20)")
    elif word_count > 30:
        warnings.append(f"Caption is too long ({word_count} words, recommended 10-20)")
    
    # Check for angle mention
    if "view" not in caption:
        warnings.append("Caption should mention view angle (front view/side view/back view)")
